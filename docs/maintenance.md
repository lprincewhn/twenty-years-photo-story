...
mock provider 会在明信片、双时空旁白、旧照片拟人和时光相遇四种叙事角度间轮换，避免连续体验得到相同标题和正文。接入真实生成模型时应同时采用以下措施，而不是只依赖提高随机参数：

1. 提示词明确提供叙事视角、场景、开篇方式和主题等可选维度，每次请求随机组合，并要求模型避免固定套话。
2. 在供应商支持时使用适度的 `temperature` 和 `top_p`，保留合理创造性；具体值用评估集调优，不能牺牲敏感内容约束。
3. 对标题及正文做标准化后的重复度检测；与本进程近期结果高度相似时更换叙事组合并限次重试，失败时明确返回 provider 错误，不静默复用旧故事。
4. 多样性不得突破差异类别允许列表、固定 AI 标签和免责声明，也不得把故事、人物名或生成输入写入日志。

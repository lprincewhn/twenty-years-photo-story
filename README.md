...
人物元数据位于 `apps/server/src/people.json`，图片位于 `apps/web/public/people/`。默认 SVG 是自制占位插画。导入真实材料前必须取得可审计的用途授权、记录来源和删除期限，并由人工检查资料与授权一一对应。完整步骤见 `docs/people-library.md`。

...
服务不创建上传目录、数据库记录、分析缓存或照片内容日志。为排查和优化，服务会输出 `experience.story_generated` 日志，包含故事标题/正文、匹配旧照标识与相对文件名、分数和统计，以 `requestId` 关联；不记录上传照片、人脸特征或令牌。授权页明确说明此用途，日志访问及留存要求见 [维护文档](docs/maintenance.md#故事与匹配照片优化日志)。Multer 使用内存存储，编排逻辑在 `finally` 中尽力清零 Buffer。浏览器对象 URL 在重拍、重新体验或组件卸载时撤销。JavaScript/操作系统仍可能短期管理内存副本，因此生产主机必须按敏感工作负载加固，禁止交换/崩溃转储泄露并配置最少日志。
